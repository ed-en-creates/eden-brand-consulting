...
const FASHION_IMAGES = Array.from({ length: 25 }, (_, i) =>
  `/fashion/fashion-${String(i + 1).padStart(2, '0')}.jpg`
)
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      result.push({
        ...
        color: new THREE.Color().setHSL(Math.random() * 0.1 + 0.6, 0.8, 0.6 + Math.random() * 0.3),
      })
    }
    return result
  }, [])
